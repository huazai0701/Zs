...
contract BNBTokenV2 is ERC20, ERC20Permit, ReentrancyGuard {
    // 所有者
    ...
    // 事件
    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    event WalletsUpdated(address indexed marketingWallet, address indexed liquidityWallet);
    ...
    constructor(
        ...
        if (_liquidityWallet == address(0)) revert InvalidLiquidityWallet();
        if (_totalSupply > type(uint256).max / (10 ** decimals())) revert TotalSupplyTooLarge();
        
        uint256 totalSupplyWithDecimals = _totalSupply * (10 ** decimals());
        
        owner = msg.sender;
        marketingWallet = _marketingWallet;
        liquidityWallet = _liquidityWallet;
        
        // 分配代币：70%给所有者，20%给营销钱包，10%给流动性钱包
        uint256 ownerAmount = (totalSupplyWithDecimals * 70) / 100;
        uint256 marketingAmount = (totalSupplyWithDecimals * 20) / 100;
        uint256 liquidityAmount = (totalSupplyWithDecimals * 10) / 100; // 明确计算10%
        
        // 验证分配总和等于总供应量，防止舍入错误
        require(ownerAmount + marketingAmount + liquidityAmount == totalSupplyWithDecimals, "Distribution math error");
    ...
    function setWallets(address _marketingWallet, address _liquidityWallet) external {
        require(msg.sender == owner, "Not owner");
        if (_marketingWallet == address(0)) revert InvalidMarketingWallet();
        if (_liquidityWallet == address(0)) revert InvalidLiquidityWallet();
        ...
        emit WalletsUpdated(_marketingWallet, _liquidityWallet);
    }
